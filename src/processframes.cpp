...
static void
	CombineGlottalAndFormants(unsigned char phase1, unsigned char phase2, unsigned char phase3, unsigned char Y) {
	unsigned int tmp;

	tmp = sam::render_tables::multtable[sam::render_tables::sinus[phase1] | amplitude1[Y]];
	tmp += sam::render_tables::multtable[sam::render_tables::sinus[phase2] | amplitude2[Y]];
	tmp += tmp > 255 ? 1 : 0; // if addition above overflows, we for some reason add one;
	tmp += sam::render_tables::multtable[sam::render_tables::rectangle[phase3] | amplitude3[Y]];
	tmp += 136;
