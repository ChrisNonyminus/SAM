...
static unsigned char RenderVoicedSample(unsigned short hi, unsigned char off, unsigned char phase1) {
	do {
		unsigned char bit	 = 8;
		unsigned char sample = sam::render_tables::sampleTable[hi + off];
		do {
...
void RenderSample(unsigned char *mem66, unsigned char consonantFlag, unsigned char mem49) {
	...
	} else RenderUnvoicedSample(hi, pitchl ^ 255, sam::render_tables::tab48426[hibyte]);
}
...
static void CreateFrames() {
	...
	while (i < 256) {
		...
		phase1 = sam::render_tables::tab47492[stressOutput[i] + 1];
		...
		do {
			frequency1[X] = sam::render_tables::freq1data[phoneme]; // F1 frequency
			frequency2[X] = sam::render_tables::freq2data[phoneme]; // F2 frequency
			frequency3[X] = sam::render_tables::freq3data[phoneme]; // F3 frequency
			amplitude1[X] = sam::render_tables::ampl1data[phoneme]; // F1 amplitude
			amplitude2[X] = sam::render_tables::ampl2data[phoneme]; // F2 amplitude
			amplitude3[X] = sam::render_tables::ampl3data[phoneme]; // F3 amplitude
			sampledConsonantFlag[X] =
				sam::render_tables::sampledConsonantFlags[phoneme]; // phoneme data for sampled consonants
...
void SetMouthThroat(unsigned char mouth, unsigned char throat) {
	...
	static const unsigned char mouthFormants5_29[30] = {0,	0,	0,	0,	0,	10, 14, 19, 24, 27, 23, 21, 16, 20, 14,
	...
	while (pos < 30) {
		...
		sam::render_tables::freq1data[pos] = newFrequency;

		// recalculate throat frequency
		initialFrequency = throatFormants5_29[pos];
		if (initialFrequency != 0) newFrequency = trans(throat, initialFrequency);
		sam::render_tables::freq2data[pos] = newFrequency;
		pos++;
	}
	...
	while (pos < 6) {
		...
		sam::render_tables::freq1data[pos + 48] = trans(mouth, initialFrequency);

		// recalculate F2 (throat formant)
		initialFrequency						= throatFormants48_53[pos];
		sam::render_tables::freq2data[pos + 48] = trans(throat, initialFrequency);
		pos++;
	}
}
