...
void Sam::setInput(const std::string &_input) {
	input = _input;
}
...
void Sam::createFrames() {
	...
	for (int i = 0; i < phonemeSize; ++i) {
		...
		if (phonemeIndexOutput[i] == PHONEME_PERIOD) {
			addInflection(RISING_INFLECTION, tableWritePosition);
		} else if (phonemeIndexOutput[i] == PHONEME_QUESTION) {
			addInflection(FALLING_INFLECTION, tableWritePosition);
		}
...
void Init() {
	...
	phonemeindex[255] =
		END; //to prevent buffer overflow // ML : changed from 32 to 255 to stop freezing with long inputs
}
